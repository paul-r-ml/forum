...
// Setting the version of FluxBB is not right for that plugin version
$valid_version = false;
...
// Looking if FluxBB 1.4.6 is running
if (version_compare($pun_config['o_cur_version'], '1.4.6') >= 0) $valid_version = true;
...
if (isset($_POST['style_change'])) {
    ...
    $fp = fopen(PUN_ROOT.'plugins/ezbbc/config.txt', 'wb');
...
if (isset($_POST['enable']) || $first_install){
        ...
        @chmod (PUN_ROOT.'plugins/ezbbc/config.txt', 0640);
        
/* Looking if the files are writable */
if (is_writable(PUN_ROOT.'header.php') && is_writable(PUN_ROOT.'include/parser.php') && is_writable(PUN_ROOT.'plugins/ezbbc/config.txt')):
...
// FluxBB 1.4.6 not running
} else {
	$ezbbc_plugin_status = '<span style="color: red; font-weight: bold;">'.$lang_ezbbc['Wrong version'].'</span>';
	$ezbbc_install_date = '####';
}
